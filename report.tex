...
method is wrong'' from ``the model is weak''. Every figure comes from \texttt{run.py}
and the charts are in \texttt{results/}. The headline metric is Total Variation
...
floor (dashed). Green bars clear the floor; red bars miss it; blue bars are
verbalized estimates (not pass/fail against the floor).}
\label{fig:scorecard}
\end{figure}
...
Per-faction splits are in \texttt{fig\_minister\_liability\_framing.png}.
...
\begin{itemize}
  ...
  \item \textbf{Single-select.} Forced through the model's structured-output / enum mode, with option order shuffled per call to cancel position bias. We sample one answer per persona at temperature above zero, one person and one answer, rather than reading an option-probability distribution off the model. Sampling is what gives the naive baseline a real spread to measure. (\texttt{--smoke} checks that sampling actually varies under the chosen backend before the full run, since constrained decoding can otherwise collapse to a single answer.)
  \item \textbf{Framing experiment.} The frame is prepended to the question as a message the respondent just encountered, and the same personas re-answer under each frame, cached separately. A neutral frame is the control, so the reported movement isolates message content rather than the mere presence of a preamble. Net support is the share choosing either ``support'' option.
  \item \textbf{Verbalized distribution.} Instead of sampling, each persona returns a probability across the options as structured JSON, which we average. It's scored and charted as an estimate rather than a sample, because a smooth average has lower variance than 100 hard votes, so it's held out of the noise-floor comparison and judged only on how well it recovers the real proportions.
  \item \textbf{Model and cost.} Provider-agnostic (Gemini or Claude behind one interface). Results here are Gemini Flash-Lite (all five questions) and Gemini 2.5 Flash (the stronger-model cross-check), on a paid Tier-1 key, about €3 in total. Responses are cached on disk, so a run resumes after a rate-limit and re-runs cost nothing. Running the same personas through a second model is the cross-model robustness check (Bisbee 2024), and it's what separated the model-driven collapse from the method itself.
